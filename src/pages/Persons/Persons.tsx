      ...
      <Spin spinning={isLoading}>
        {persons.length === 0 && !isLoading ? (
          ...
          <Row gutter={rowGutter} >
            {persons.map(({ name, url, gender,birth_year,starships,vehicles }) => (
              <PersonCard name={name} gender={gender} url={url} birthyear={birth_year} starships={starships.length} vehicles={vehicles.length} key={url} />
            ))}
          </Row>
        )}
      </Spin>
